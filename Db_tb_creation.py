...
class Student:
    def __init__(self,root):
        self.root=root
        self.root.title("Database and Table Creation")
        ...
        lbl_nm=Label(Manage_Frame,text="Name of New Database",bg="Orange",fg="White", font=("times new roman",20,"bold"))
        ...
        #================Button=================================================
        Submitbtn=Button(root,text="Submit",bd=5,bg='Light Green',fg='Blue',font=("Algerian",15,"bold"),width=20,command=self.Submit)
        Submitbtn.place(x=550,y=400)
    

    def Submit(self):
        if str(self.Password.get()).lstrip()!="" and self.port.get()!=0 and str(self.n_table.get()).lstrip()!="" and str(self.name.get()).lstrip()!="":
        #   and str(self.database.get()).lstrip()!="":

            try:
                mydb=mysql.connector.connect(host="localhost",user="root",port=int(self.port.get()),password=self.Password.get())
                mycursor=mydb.cursor()
                mycursor.execute("CREATE DATABASE "+self.name.get())
                self.table_creation()
                mydb.commit()
                mydb.close()
                messagebox.showinfo("Created","Database successfully created")
    ...
    def table_creation(self):
        try:
            ...
            mycursor=mydb.cursor()
            mycursor.execute("CREATE TABLE "+self.n_table.get()+" (Adm_No int(100)Primary Key ,name varchar(100),class varchar(100), gender varchar(100),father varchar(100),contact varchar(100),dob varchar(100),school varchar(100),address varchar(200))")
            mydb.commit()
            mydb.close()
